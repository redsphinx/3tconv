...
def make_download_list(mode, which, start, end):
    pass
...
def add_success(vid_id):
    pass


def remove_from_failed(vid_id):
    pass


def add_failed_reason(video_id, opt_reason):
    pass
...
def run(mode, which, start, end):
    ...
    download_list = make_download_list(mode, which, start, end)
    download_list.sort()
    ...
    for i in tqdm(range(start, end)):
        ...
        if is_success:
            add_success(video_id)
            if mode == 'only_failed':
                remove_from_failed(video_id)
        else:
            assert opt_reason is not None
            add_failed_reason(video_id, opt_reason)
